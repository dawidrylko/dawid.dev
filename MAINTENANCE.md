...
| `quartz/plugins/emitters/sitemap.ts`, `quartz/components/custom/noindexRoutes.ts` | Own sitemap emitter plus the list of slugs kept out of the index; `ContentIndex` runs with `enableSiteMap: false` because upstream has no per-page filter and emitters run in parallel, so its output cannot be rewritten afterwards | After a bump, diff `generateSiteMap` in `quartz/plugins/emitters/contentIndex.tsx` against the copy here and port any format change; confirm `ContentIndex` still accepts `enableSiteMap`, and that its `includeEmptyFiles` still defaults to true (the owned emitter does not mirror that filter) |
| `quartz/plugins/emitters/rss.tsx` | Own `index.xml` emitter, same reason and same slug list as the sitemap: `ContentIndex` runs with `enableRSS: false` so the legal pages stop occupying two of the feed's ten slots. It also re-attaches the RSS autodiscovery head link, which `ContentIndex` stops emitting once `enableRSS` is off | After a bump, diff `generateRSSFeed` and the `externalResources` head link in `quartz/plugins/emitters/contentIndex.tsx` against the copies here and port any format change; confirm `ContentIndex` still accepts `enableRSS`. Three upstream options are deliberately not carried over (`rssFullHtml`, `rssSlug`, `includeEmptyFiles`), so setting any of them on `ContentIndex` will silently fail to reach the feed |
| `quartz/styles/custom.scss` | Quartz's official user-CSS extension point | Verify it is still imported the same way. The narrow-viewport sidebar rule leans on upstream spacing, so re-measure a 360px viewport after a bump |
